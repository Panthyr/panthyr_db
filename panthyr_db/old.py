...
class connection(sqlite3.Connection):
    ...
    def set_setting(self, setting, value):
        ...
        try:
            ...
            self.__c.execute('insert or ignore into settings(setting) VALUES(?)', (setting, ))
    ...
    def add_to_queue(self, action, priority='2', options=''):
        ...
        try:
            ...
            err_str = 'Error while adding task ({},{},{}) to queue table: {}'.format(
                action, priority, options, e)
            log.error(err_str)
    ...
    def get_next_task(self, **kwargs):
        ...
        """
        self.__c.execute(
            'select id, priority, action, options, fails from queue where done == 0 and priority == 1 and fails < 3 order by id limit 1'
        )
        ...
        if type(reply) == tuple:
            ...
            self.__c.execute(
                'select id, priority, action, options, fails from queue where done == 0 and priority == 2 and fails < 3 order by id limit 1'
            )
    ...
    def get_all_tasks(self):
        ...
        '''
        self.__c.execute(
            'select priority, id, action, options from queue where done == 0 order by priority asc, id asc'
        )
    ...
    def get_number_of_tasks(self, done=0):
        ...
        '''
        try:
            self.__c.execute('select count() from queue where done == ? and fails < 3',
                             (str(done)))
            reply = self.__c.fetchone()  # returns a tuple, we only need the first item
            # returns the first item of the returned tuple
            return (True, int(reply[0]))
    ...
    def get_protocol(self):
        ...
        try:
            self.__c.execute(
                'SELECT instrument, zenith, azimuth, repeat, wait from protocol ORDER BY number')
    ...
    def add_meas(self, meas_dict):
        ...
        logged_items = [
            'timestamp', 'valid', 'setup_error', 'cycle_id', 'gnss_acquired', 'gnss_qual',
            'gnss_lat', 'gnss_lon', 'batt_voltage', 'head_voltage', 'head_temp_hpt', 'cycle_scan',
            'prot_sensor', 'prot_zenith', 'prot_azimuth', 'sun_heading', 'sun_elevation',
            'scan_heading', 'scan_error', 'scan_rep', 'rep_error', 'rep_unix', 'rep_serial'
        ]
        d = dict()

        for i in logged_items:
            if i in meas_dict:  # check if it is in the provided dict
                # these are lists to accomodate multiple errors
                if i in ('scan_error', 'setup_error'):
                    import pdb
                    pdb.set_trace()
                    # so join them into a string
                    d[i] = ' | '.join([str(x) for x in meas_dict[i]])
                else:
        ...
        try:
            self.execute(
                'insert into measurements({}) values ({})'.format(columns[:-2], placeholders[:-2]),
                values)
            self.__commit_db()
    ...
    def export_data(self, target_db_name, table_ids):
        ...
        try:
            assert type(
                table_ids) == list, 'tables_ids should be a list of [table,start_id,stop_id] items'
            if len(table_ids) == 0:
                raise Exception('no target tables have been asked')
            ...
            # attach the target database to our main db
            self.execute('ATTACH DATABASE ? AS target_db', (target_db_name, ))
            for line in table_ids:
                assert len(
                    line
                ) == 3, 'not enough fields, tables_ids should be a list of [table,start_id,stop_id] items'
                table, start_id, stop_id = line  # unpack list

                # base command for the export
                command = 'INSERT INTO {} SELECT * FROM {}'.format('target_db.' + table, table)
...
def create_db(db_file=database_location, id=('all', ), populate_settings=True):
    ...
    """
    if any(table not in valid_tables and not table == 'all'
           for table in id):  # there's an invalid table requested
        msg = "one of the provided tables is invalid. id should be a tuple. Valid: {} or 'all'".format(
            valid_tables)
        log.warning(msg)
        ...
        with db:  # using context manager to automatically commit or roll back changes.
            ...
            if any(x in ('measurements', 'all') for x in id):  # measurement table
                base_command = (
                    ...
                    'setup_error text collate nocase, ' + 'cycle_id text, ' + 'gnss_acquired date,'
                    'gnss_qual integer, ' + 'gnss_lat real, ' + 'gnss_lon real, ' +
                    'batt_voltage,' + 'head_voltage real, ' + 'head_temp_hpt text, ' +
                    'cycle_scan integer, ' + 'prot_sensor text, ' + 'prot_zenith integer, ' +
                    'prot_azimuth integer,' + 'sun_heading real, ' + 'sun_elevation real, ' +
                    'scan_heading real, ' + 'scan_error text, ' + 'scan_rep integer, ' +
                    'rep_error text, ' + 'rep_unix real, ' + 'rep_serial text, ')
                for i in range(1, 257):
                    # add the 256 value columns
                    base_command += 'val_{:03d} integer, '.format(i)
                # remove the comma and space at the end, replace by closing brackets
                complete_command = base_command[:-2] + ')'
            ...
            if any(x in ('protocol', 'all') for x in id):  # protocol table
                db.execute('create table protocol(id integer primary key autoincrement, ' +
                           ...
                           'zenith integer not null, ' + 'azimuth integer not null, ' +
                           'repeat integer not null default 1, ' +
                           'wait integer not null default 0)')

            if any(x in ('settings', 'all') for x in id):  # settings table
                db.execute(
                    'create table settings(setting text primary key not null collate nocase, ' +
                    'value text collate nocase)')
                if populate_settings:  # don't set default settings if this is a db for exporting data
                    default_settings = (('station_id', 'MSO'), ('manual',
                                                                1), ('measurements_start_hour', 6),
                                        ('measurements_stop_hour', 19), ('max_sun_zenith', 90),
                                        ('email_enabled', 1), ('email_recipient',
                                                               ''), ('email_server_port', ''),
                                        ('email_user', ''), ('email_password',
                                                             ''), ('email_min_level', 'warning'),
                                        ('ftp_server', ''), ('ftp_user', ''), ('ftp_password', ''),
                                        ('ftp_working_dir',
                                         'hypermaq'), ('head_true_north_offset',
                                                       180), ('radiance_angle_offset',
                                                              20), ('irradiance_angle_offset', 60),
                                        ('keepout_heading_low', 0), ('keepout_heading_high',
                                                                     0), ('gnss_acquired', 'none'),
                                        ('gnss_lat', 51.2), ('gnss_lon', 2.9), ('gnss_qual', 0),
                                        ('gnss_mag_var', 0), ('id_last_backup_meas',
                                                              0), ('id_last_backup_log',
                                                                   0), ('system_set_up', 0))
                    db.executemany('insert into settings(setting,value) values (?, ?)',
                                   (default_settings))
...
def export_table_csv(table='measurements', date=False):
    ...
    try:
        if not (table in valid_tables):
            raise Exception('Invalid target table for export: {} (should be one of {}'.format(
                table, valid_tables))
        ...
        if table in (tables_with_timestamp):
            if date:  # a date was supplied as argument
                try:
                    ...
                    raise Exception(
                        'Invalid date provided: "{}" (should be YYYY-MM-DD)'.format(date))
            else:  # no date argument supplied
                target = datetime.now()

            filename_base = 'export_day_{tablename}_{date}'.format(tablename=table,
                                                                   date=target.strftime('%Y%m%d'))
            sqlite_query = 'select * from {} where date(timestamp) = "{}";'.format(
                table, target.strftime('%Y%m%d'))  # prepare the sqlite query

        else:  # table without timestamp column
            filename_base = 'export_current_{tablename}_{date}'.format(
                tablename=table, current_timestamp=target.strftime('%Y%m%d'))
            # prepare the sqlite query
            sqlite_query = 'select * from {};'.format(table)

        # check if the filename is unique
        i = 1
        filename = filename_base  # try the simpelest filename, in its base form
        # a file with that name already exists
        while os.path.isfile(os.path.join(data_path, (filename + extention))):
            # if filename already exists, add _xx at the end, starting at number 01
            filename = '{}_{:02d}'.format(filename_base, i)
            i += 1  # and increment until filename is unique

        complete_filename = os.path.join(data_path, (filename + extention))

        # context manager handles file closing in case of problems
        with open(complete_filename, 'w') as outputfile:
            call([
                sqlite3_full_path, sqlite3_option1, sqlite3_option2, database_location,
                sqlite_query
            ],
                 stdout=outputfile)
    ...
    def populate_credentials(self,
                             cf_location='/home/hypermaq/data/credentials',
                             credentials=('email_user', 'email_password', 'email_server_port',
                                          'ftp_server', 'ftp_user', 'ftp_password')):
        ...
        try:
            parsed_credentials = get_credentials(cf_location=cf_location,
            ...
            for c in credentials:
                try:
                    ...
                    log.error('could not get credentials info for {}: {}'.format(c, e))
